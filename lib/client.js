...
function EditorArea(props) {
  ...
  return React.createElement(React.Fragment, null,
    ...
    crumbBase ? React.createElement("div", { className: "sb-breadcrumbs" },
      ...
      React.createElement("span", { className: "sb-spacer" }),
      // 自动保存开启时内容已自动落盘,无需手动保存按钮
      activeContent && activeContent.dirty && !editorSettings.autoSave ? React.createElement("button", { className: "sb-save", onClick: onSave }, "保存") : null,
...
function SidebarPanel(props) {
  ...
  function ctxAddContext() {
    ...
    var findComposer = function () {
      // composer 的受控 textarea 带 data-phase 属性(其它输入框没有)
      return document.querySelector('textarea[data-phase]')
        || document.querySelector('[data-conversation-composer] textarea')
        || document.querySelector('[data-slot="conversation.session"] textarea')
        || document.querySelector('textarea');
    };
    try {
      var ta = findComposer();
      if (!ta) { setMsg("未找到对话输入框"); return; }
      // 受控 textarea:用原生 value setter 绕过 React 值追踪,再派发 input 事件让 React 采纳
      var suffix = rel + (ta.value && !/\s$/.test(ta.value) ? " " : "");
      var setter = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, "value").set;
      setter.call(ta, ta.value + suffix);
      ...
      // 兜底:模拟粘贴事件(部分环境走这条路径)
      try {
        var ta2 = findComposer();
        if (!ta2) { setMsg("未找到对话输入框"); return; }
        var dt = new DataTransfer();
        dt.setData("text/plain", rel);
        ta2.dispatchEvent(new ClipboardEvent("paste", { bubbles: true, cancelable: true, clipboardData: dt }));
        ta2.focus();
        setMsg("已引用文件路径: " + rel);
      } catch (err2) {
        setMsg("添加失败: " + String(err2 && err2.message ? err2.message : err2));
      }
    }
  }
      function ctxCopyRel() {
