...
function editorThemeName() {
  ...
  else if (typeof window !== "undefined" && window.matchMedia) dark = window.matchMedia("(prefers-color-scheme: dark)").matches;
...
function SidebarPanel(props) {
  ...
  if (sessions && sessions.current && sessions.byId && sessions.byId[sessions.current]) {
    curCwd = sessions.byId[sessions.current].cwd || null;
  }
  ...
  function loadGit() {
    if (!root) return;
    fetch("/dsh-gitbar/status?cwd=" + encodeURIComponent(root))
      .then(function (r) { return r.json(); })
      .then(function (d) { setGitData((d && d.ok && d.repo) ? d : null); })
      .catch(function () { setGitData(null); });
  ...
  function gitGenAi() {
    ...
    setGitBusy(true);
    setGitResult(null);
    fetch("/dsh-gitbar/ai?cwd=" + encodeURIComponent(root)).then(function (r) { return r.json(); }).then(function (d) {
  ...
  function createTermFor(tab) {
    ...
    var ws = new WebSocket(proto + "//" + window.location.host + "/dsh-termbar/ws?cwd=" + encodeURIComponent(tab.cwd || "") + "&shell=" + encodeURIComponent(tab.shell || "bash"));
  ...
  function bootTerm(tab) {
    ...
    link.rel = "stylesheet";
    link.href = "/dsh-termbar/xterm.css";
    document.head.appendChild(link);
    var s = document.createElement("script");
    s.src = "/dsh-termbar/xterm.js";
    s.onload = function () {
    ...
    s.onerror = function () { xtermBootRef.current = false; };
    document.head.appendChild(s);
  }
  ...
  React.useEffect(function () {
    function onSettings() {
      ...
      try {
        ...
        var m = monacoInst.current.getModel();
        if (m) window.monaco.editor.setModelLanguage(m, editorSettings.highlight ? langOf(activeTab ? activeTab.name : "") : "plaintext");
        attachMinimapMenu();
      } catch (e) {}
    }
    window.addEventListener("dsh-editor-settings", onSettings);
    return function () { window.removeEventListener("dsh-editor-settings", onSettings); };
  }, []);
  ...
  function renderNode(e, p, depth) {
    ...
    return React.createElement("div", { key: full },
      React.createElement("div", {
        ...
        style: style,
        title: full,
        onClick: function () { if (isDir) toggleDir(full, e.name); else openFile(full, e.name); },
        onContextMenu: function (ev) { ctxOpen(ev, full, e.name, isDir); }
      },
  ...
  // 文件树 memo(输入内容变化时不重渲染整棵树;必须在条件返回前调用,保持 hooks 数量一致)
  var treeView = React.useMemo(function () {
  ...
  var rootChildren = root ? tree[root] : null;
    ...
    React.createElement("div", { className: "sb-panel", ref: panelRef, style: { width: width } },
      ...
      React.createElement("div", { className: "sb-main" },
        React.createElement("div", { className: "sb-editor" },
          ...
          React.createElement("div", { className: "sb-tabs" },
            tabs.map(function (t, idx) {
              ...
              return React.createElement("span", {
                ...
                onClick: function () { setActivePath(t.path); },
                title: t.path,
                draggable: true,
                onDragStart: function (e) { tabDragStart(idx, e); },
                onDragOver: function (e) { tabDragOver(idx, e); },
                onDrop: function (e) { tabDrop(idx, e); },
                onDragEnd: function () { dragTabRef.current = null; setDragHover(null); },
                onContextMenu: function (e) { tabOpenMenu(e, t.path); }
              },
                React.createElement("i", { className: fi[0], style: { color: fi[1] } }),
                React.createElement("span", null, t.name + (c && c.dirty ? " ●" : "")),
                React.createElement("span", { className: "sb-tab-x", onClick: function (e) { e.stopPropagation(); closeTab(t.path); } }, "\u2715")
              );
            }),
            ...
            crumbs.map(function (c, i) {
              return React.createElement("span", { key: i, style: { display: "inline-flex", alignItems: "center" } },
                ...
                React.createElement("span", {
                  ...
                  title: c.path,
                  onClick: function () { if (!c.locked && !c.isFile) toggleDir(c.path, c.name); }
                }, c.name)
              );
            }),
            React.createElement("span", { className: "sb-msg", style: { margin: "0 6px" } }, msg),
            React.createElement("span", { className: "sb-spacer" }),
            activeContent && activeContent.dirty ? React.createElement("button", { className: "sb-save", onClick: saveActive }, "保存") : null,
            activeIsMd ? React.createElement("button", { className: "sb-iconbtn", title: mdPreview ? "切换到代码视图" : "切换到预览视图", onClick: function () { setMdPreview(!mdPreview); } },
              React.createElement("i", { className: mdPreview ? "ri-code-s-slash-line" : "ri-eye-line" })
            ) : null,
            React.createElement("button", { className: "sb-iconbtn", title: "在 VS Code 中打开", onClick: function () { openInVscode(activePath); } },
              React.createElement("img", { src: "/dsh-sidebar/vscode.svg", width: 15, height: 15, alt: "VS Code", style: { display: "block" } })
            ),
            React.createElement("button", { ref: moreRef, className: "sb-iconbtn", title: "更多", onClick: toggleMoreMenu }, React.createElement("i", { className: "ri-more-fill" }))
          ...
          React.createElement("div", { ref: termPanelRef, className: "sb-term-panel", style: { display: termOpen ? undefined : "none", height: termHeight } },
            ...
            React.createElement("div", { className: "sb-term-head" },
              ...
              termTabs.map(function (t) {
                return React.createElement("span", { key: t.id, className: "sb-term-tab" + (t.id === termActive ? " active" : ""), onClick: function () { setTermActive(t.id); }, title: t.cwd || "" },
                  React.createElement("i", { className: "ri-terminal-line" }),
                  React.createElement("span", null, t.label),
                  React.createElement("span", { className: "sb-term-tab-x", onClick: function (e) { e.stopPropagation(); closeTermTab(t.id); } }, "\u2715")
                );
              }),
              React.createElement("span", { className: "sb-spacer" }),
              React.createElement("div", { className: "sb-term-capsule" },
                React.createElement("button", { ref: termTypeRef, className: "sb-term-type", title: "终端类型(用于新终端)", onClick: toggleTermType },
                ...
                React.createElement("button", { className: "sb-term-capsule-new", title: "新建终端", onClick: function () { newTerm(); } }, React.createElement("i", { className: "ri-add-line" }))
              ),
              React.createElement("button", { className: "sb-term-close", title: "关闭终端面板", onClick: function () { setTermOpen(false); } }, React.createElement("i", { className: "ri-close-line" }))
            ),
            termMenu ? React.createElement("div", { className: "sb-menu", ref: termMenuRef, style: { top: termMenu.y, left: termMenu.x } },
              React.createElement("div", { className: "sb-menu-item", onClick: function () { pickTermShell("bash"); } }, "Bash" + (termShell === "bash" ? " ✓" : "")),
              React.createElement("div", { className: "sb-menu-item", onClick: function () { pickTermShell("pwsh"); } }, "PowerShell" + (termShell === "pwsh" ? " ✓" : ""))
            ) : null,
          ...
          React.createElement("div", { className: "sb-status" },
            React.createElement("button", { className: "sb-st-seg" + (hasEditorFile ? "" : " off"), title: "转到行列", onClick: function () { if (hasEditorFile) stOpen("gotoline"); } }, hasEditorFile ? "行" + cursorPos.line + "列" + cursorPos.col : ""),
            React.createElement("button", { className: "sb-st-seg" + (hasEditorFile ? "" : " off"), title: "选择缩进", onClick: function () { if (hasEditorFile) stOpen("indent"); } }, hasEditorFile ? "空格:" + (indentMode === "tab" ? "Tab" : indentSize) : ""),
            React.createElement("button", { className: "sb-st-seg" + (hasEditorFile ? "" : " off"), title: "选择编码", onClick: function () { if (hasEditorFile) stOpen("encoding"); } }, hasEditorFile ? encState : ""),
            React.createElement("button", { className: "sb-st-seg" + (hasEditorFile ? "" : " off"), title: "选择行尾序列", onClick: function () { if (hasEditorFile) stOpen("eol"); } }, hasEditorFile ? eolState : ""),
            React.createElement("button", { className: "sb-st-seg" + (hasEditorFile ? "" : " off"), title: "选择语言模式", onClick: function () { if (hasEditorFile) stOpen("lang"); } },
        ...
        React.createElement("div", { className: "sb-explorer", style: { width: explorerW } },
          ...
          React.createElement("div", { className: "sb-ex-body" },
            React.createElement("div", { style: { display: exTab === "files" ? "flex" : "none", flex: 1, minHeight: 0, flexDirection: "column" } },
              ...
              React.createElement("div", { className: "sb-ex-tree" }, treeView)
            ),
            React.createElement("div", { className: "sb-search", style: { display: exTab === "search" ? undefined : "none" } },
              React.createElement("div", { className: "sb-search-head" },
                React.createElement("span", { className: "sb-search-title" }, "搜索"),
                React.createElement("span", { className: "sb-spacer" }),
                React.createElement("button", { className: "sb-iconbtn", title: "重新搜索", onClick: doSearch }, React.createElement("i", { className: "ri-refresh-line" })),
                React.createElement("button", { className: "sb-iconbtn", title: "清除结果", onClick: clearSearch }, React.createElement("i", { className: "ri-close-circle-line" })),
                React.createElement("button", { className: "sb-iconbtn", title: "新建搜索", onClick: clearSearch }, React.createElement("i", { className: "ri-file-add-line" }))
              ),
              ...
              React.createElement("div", { className: "sb-res-head" },
                ...
                searchRes && searchRes.length ? (
                  searchOpenOnly
                    ? React.createElement("span", null,
                        " - ",
                        React.createElement("span", { className: "sb-res-link on", title: "仅在打开的文件中搜索", onClick: function () { toggleOpenOnly(false); } },
                          "仅在打开的文件中搜索(禁用)"),
                        " - ",
                        React.createElement("span", { className: "sb-res-link", title: "在编辑器中打开", onClick: openSearchEditor }, "在编辑器中打开")
                      )
                    : React.createElement("span", null,
                        " - ",
                        React.createElement("span", { className: "sb-res-link", title: "在编辑器中打开", onClick: openSearchEditor }, "在编辑器中打开")
                  ...
                  (!searchGroups ? React.createElement("div", { className: "sb-node", style: { color: "var(--dsw-alias-label-tertiary)" } }, "无结果") :
                    resFlat.slice(resWin.start, resWin.end).map(function (it, idx) {
                      if (it.t === "g") {
                        ...
                        return React.createElement("div", { key: "g" + p, className: "sb-sr-file" + (it.first ? "" : " sb-sr-ghead"), title: p, onClick: function () { openFileAt(p, baseName(p), null); } },
                          React.createElement("span", { className: "sb-sr-chev", onClick: function (e) { e.stopPropagation(); toggleGroup(p); } },
                          ...
                          React.createElement("span", { className: "sb-sr-actions" },
                            detailsOpen ? React.createElement("button", { className: "sb-iconbtn", title: "全部替换", onClick: function (e) { e.stopPropagation(); doReplaceFile(p); } }, React.createElement("i", { className: "ri-exchange-line" })) : null,
                            React.createElement("button", { className: "sb-iconbtn", title: "全部排除", onClick: function (e) { e.stopPropagation(); excludeFile(p); } }, React.createElement("i", { className: "ri-close-line" }))
                          )
                        );
                      }
                      var r = it.r;
                      return React.createElement("div", { key: "r" + r.path + ":" + (r.line || 0) + ":" + idx, className: "sb-sr-item", title: r.path + (r.line ? ":" + r.line : ""), onClick: function () { openFileAt(r.path, r.name, r.line || null); } },
                        ...
                        React.createElement("span", { className: "sb-sr-actions" },
                          detailsOpen ? React.createElement("button", { className: "sb-iconbtn", title: "替换", onClick: function (e) { e.stopPropagation(); doReplaceLine(r.path, r.line); } }, React.createElement("i", { className: "ri-exchange-line" })) : null,
                          React.createElement("button", { className: "sb-iconbtn", title: "排除", onClick: function (e) { e.stopPropagation(); excludeFile(r.path); } }, React.createElement("i", { className: "ri-close-line" }))
                        )
                      );
                    })
                  )
            ...
            React.createElement("div", { className: "sb-git", style: { display: exTab === "git" ? undefined : "none" } },
              gitData ? React.createElement(React.Fragment, null,
                React.createElement("div", { className: "sb-git-branchrow" },
                  React.createElement("div", { className: "sb-git-branch" },
                    ...
                    React.createElement("span", null, gitData.branch + ((gitData.ahead || gitData.behind) ? " ↑" + gitData.ahead + " ↓" + gitData.behind : ""))
                  ),
                  React.createElement("span", { className: "sb-spacer" }),
                ...
                React.createElement("div", { className: "sb-git-actions" },
                  React.createElement("button", { className: "sb-git-act", title: "Git 操作", onClick: gitOpenModal }, "操作")
                )
              ) : React.createElement("div", { className: "sb-node", style: { color: "var(--dsw-alias-label-tertiary)" } }, "未检测到 Git 仓库")
            )
          )
        )
      ),
      moreOpen && morePos ? React.createElement("div", { className: "sb-menu", ref: moreMenuRef, style: { top: morePos.top, left: morePos.left } },
        React.createElement("div", { className: "sb-menu-item", onClick: function () { copyPath(true); } }, "复制绝对路径"),
        React.createElement("div", { className: "sb-menu-item", onClick: function () { copyPath(false); } }, "复制相对路径"),
        React.createElement("div", { className: "sb-menu-item", onClick: revealInExplorer }, "在资源管理器打开")
      ) : null,
      mmMenu ? React.createElement("div", { className: "sb-menu", ref: mmRef, style: { top: mmMenu.y, left: mmMenu.x } },
        React.createElement("div", { className: "sb-menu-item" + (editorSettings.minimap ? " on" : ""), onClick: function () { mmSet({ minimap: !editorSettings.minimap }); } },
          React.createElement("span", null, "切换缩略图")),
        React.createElement("div", { className: "sb-menu-sep" }),
        React.createElement("div", { className: "sb-menu-item", onMouseEnter: function (e) { mmSubOpen("size", e); }, onMouseLeave: mmSubClose },
          React.createElement("span", null, "缩略图大小"),
          React.createElement("i", { className: "ri-arrow-right-s-line sb-subarrow" })),
        React.createElement("div", { className: "sb-menu-item", onMouseEnter: function (e) { mmSubOpen("slider", e); }, onMouseLeave: mmSubClose },
          React.createElement("span", null, "滑块显示"),
          React.createElement("i", { className: "ri-arrow-right-s-line sb-subarrow" })),
        React.createElement("div", { className: "sb-menu-sep" }),
        React.createElement("div", { className: "sb-menu-item" + (editorSettings.renderCharacters ? " on" : ""), onClick: function () { mmSet({ renderCharacters: !editorSettings.renderCharacters }); } },
          React.createElement("span", null, "渲染字符"))
      ) : null,
      mmSub ? React.createElement("div", { className: "sb-menu", ref: mmSubEl, style: { top: mmSub.y, left: mmSub.x, minWidth: 132 }, onMouseEnter: mmSubCancel, onMouseLeave: mmSubClose },
        (mmSub.which === "size" ? [["fit", "适应高度"], ["fill", "适应宽度"], ["proportional", "按比例缩放"]] : [["always", "总是显示"], ["mouseover", "悬停时显示"]]).map(function (o) {
          ...
          return React.createElement("div", { key: o[0], className: "sb-menu-item" + (active ? " on" : ""), onClick: function () { mmSet(mmSub.which === "size" ? { minimapSize: o[0] } : { minimapSlider: o[0] }); } },
            React.createElement("span", null, o[1]));
        })
      ) : null,
      ctxMenu ? React.createElement("div", { className: "sb-menu", ref: ctxMenuRef, style: { top: ctxMenu.y, left: ctxMenu.x } },
        !ctxMenu.isDir ? React.createElement("div", { className: "sb-menu-item", onClick: ctxAct(ctxOpenInEditor) }, "在编辑器打开") : null,
        React.createElement("div", { className: "sb-menu-item", onClick: ctxAct(ctxOpenInTerm) }, "在终端打开"),
        !ctxMenu.isDir ? React.createElement("div", { className: "sb-menu-item", onClick: ctxAct(ctxAddContext) }, "添加至对话上下文") : null,
        React.createElement("div", { className: "sb-menu-sep" }),
        React.createElement("div", { className: "sb-menu-item", onClick: ctxAct(ctxCopyRel) }, "复制相对路径"),
        React.createElement("div", { className: "sb-menu-item", onClick: ctxAct(ctxCopyAbs) }, "复制绝对路径"),
        React.createElement("div", { className: "sb-menu-item", onClick: ctxAct(ctxReveal) }, "在文件管理器打开"),
        React.createElement("div", { className: "sb-menu-item", onClick: ctxAct(ctxVsc) }, "在 VS Code 打开"),
        React.createElement("div", { className: "sb-menu-sep" }),
        React.createElement("div", { className: "sb-menu-item", onClick: ctxAct(ctxRename) }, "重命名")
      ) : null,
      tabMenu ? React.createElement("div", { className: "sb-menu", ref: tabMenuRef, style: { top: tabMenu.y, left: tabMenu.x } },
        React.createElement("div", { className: "sb-menu-item", onClick: tabClose }, "关闭"),
        React.createElement("div", { className: "sb-menu-item", onClick: tabCloseOthers }, "关闭其他标签"),
        React.createElement("div", { className: "sb-menu-item", onClick: tabCloseAll }, "关闭所有标签")
      ) : null,
      stPopup ? React.createElement("div", { className: "sb-modal-mask", onMouseDown: function (e) { if (e.target === e.currentTarget) stClose(); } },
        React.createElement("div", { className: "sb-modal", ref: stMenuRef },
          React.createElement("div", { className: "sb-modal-head" },
            React.createElement("span", null, { gotoline: "转到行列", indent: "选择缩进", encoding: "选择编码", eol: "选择行尾序列", lang: "选择语言模式" }[stPopup]),
            React.createElement("span", { className: "sb-spacer" }),
            React.createElement("button", { className: "sb-iconbtn", title: "关闭", onClick: stClose }, React.createElement("i", { className: "ri-close-line" }))
          ),
          React.createElement("div", { className: "sb-modal-body" },
            stPopup === "gotoline" ? React.createElement(React.Fragment, null,
              React.createElement("input", {
                className: "sb-search-plain",
                placeholder: "行:列",
                value: goLineVal,
                onChange: function (e) { setGoLineVal(e.target.value); },
                onKeyDown: function (e) { if (e.key === "Enter") stGoToLine(); },
                autoFocus: true
              }),
              React.createElement("div", { style: { display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 12 } },
                React.createElement("button", { className: "sb-save", onClick: stGoToLine }, "转到"),
                React.createElement("button", { className: "sb-save", onClick: stClose }, "取消")
              )
            ) :
            stPopup === "indent" ? React.createElement(React.Fragment, null,
              [2, 4, 8].map(function (v) {
                return React.createElement("div", { key: v, className: "sb-modal-item" + (indentMode === "spaces" && indentSize === v ? " on" : ""), onClick: function () { stApplyIndent(v); } }, "空格:" + v);
              }),
              React.createElement("div", { className: "sb-modal-item" + (indentMode === "tab" ? " on" : ""), onClick: function () { stApplyIndent("tab"); } }, "Tab 缩进")
            ) :
            stPopup === "encoding" ? ["utf-8", "gbk", "gb18030", "big5"].map(function (enc) {
              return React.createElement("div", { key: enc, className: "sb-modal-item" + (encState === enc.toUpperCase() ? " on" : ""), onClick: function () { stApplyEncoding(enc); } }, enc.toUpperCase());
            }) :
            stPopup === "eol" ? ["LF", "CRLF"].map(function (e2) {
              return React.createElement("div", { key: e2, className: "sb-modal-item" + (eolState === e2 ? " on" : ""), onClick: function () { stApplyEol(e2); } }, e2);
            }) :
            React.createElement("div", { className: "sb-modal-grid" },
              LANG_LIST.map(function (lg) {
                ...
                return React.createElement("div", { key: lg[0] + lg[1], className: "sb-modal-item" + (curLang === lg[0] ? " on" : ""), onClick: function () { stApplyLang(lg[0], lg[1]); } }, lg[1]);
              })
            )
          )
        )
      ) : null,
      gitModalOpen ? React.createElement("div", { className: "gb-modal-mask", onClick: function (e) { if (e.target === e.currentTarget) setGitModalOpen(false); } },
        React.createElement("div", { className: "gb-modal" },
          React.createElement("div", { className: "gb-head" },
            React.createElement("span", { className: "gb-branch", onClick: function (e) { e.stopPropagation(); setGitBranchMenu(!gitBranchMenu); } },
              React.createElement("i", { className: "ri-git-branch-line" }),
              ...
              gitBranchMenu ? React.createElement("div", { className: "gb-branchmenu", onMouseDown: function (e) { e.stopPropagation(); }, onClick: function (e) { e.stopPropagation(); } },
                (gitData && gitData.branchList ? gitData.branchList : []).map(function (b) {
                  return React.createElement("div", { key: b, className: "gb-branchitem" + (b === gitBranch ? " active" : ""), onClick: function () { setGitBranch(b); setGitBranchMenu(false); } }, b);
                })
              ) : null
            ),
          ...
          React.createElement("div", { className: "gb-msgwrap" },
            React.createElement("textarea", {
              ...
              value: gitMsg,
              onChange: function (e) { setGitMsg(e.target.value); },
              onKeyDown: function (e) { if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) { e.preventDefault(); gitCommit(false); } }
            }),
          ...
          React.createElement("label", { className: "gb-opt" },
            React.createElement("input", {
              type: "checkbox",
              checked: gitInclude,
              onChange: function (e) { setGitInclude(e.target.checked); }
            }),
            "包含未暂存的更改",
          ...
          React.createElement("div", { className: "gb-btns" },
            React.createElement("button", { className: "gb-btn main", disabled: gitBusy, onClick: function () { gitCommit(false); } }, "提交"),
            React.createElement("button", { className: "gb-btn", disabled: gitBusy, onClick: function () { gitCommit(true); } }, "提交并推送"),
            React.createElement("button", { className: "gb-btn", disabled: gitBusy, onClick: gitPush }, "推送")
          ),
